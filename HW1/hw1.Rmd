...
# Analyzing series z
***Unless altered I have omitted the code from the analysis of series z.***

## 2.1. Plot time series x
...
m1 <- forecast::Arima(x, order = c(1,1,0))
print("ARIMA(1,1,0):")
print(m1)
print(Box.test(m1$residuals, lag=40, type="Ljung-Box"))
```

The Box-Ljung test shows that residuals of the series are not white noise after fitting the ARIMA(1,1,0) model, suggesting that it fits poorly.

While I know I was only supposed to present one model here I was curious as to whether an ARIMA(0,1,1) model would fit better. The plot of `z` seems to resemble a moving average, despite the difference in the PACF. The output below shows this model. We can see  that the AIC and BIC are much lower and the p-value for the Box-Ljung test is not significant, so we can assume that the residuals are white noise. This can be observed in the plot of the residuals, which still has some high variance but no longer shows the same characteristic trends of the original series.
